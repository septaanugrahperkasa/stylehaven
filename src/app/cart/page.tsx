...
export default async function Cart() {
  ...
  if (data?.products) {
    _products = await getProductsByIds(data.products.map((d) => d.product_id));

    quantity = data.products.map((p) => p.quantity).reduce((a, b) => a + b, 0);

    subtotal = _products
      .filter((p, index) => p._id === data.products[index].product_id)
      .map((p, index) => p.price * data.products[index].quantity)
      .reduce((a, b) => a + b, 0);
  }
